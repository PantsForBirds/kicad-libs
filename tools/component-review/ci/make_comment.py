...
def usage_line(review) -> str:
    u = review.get("usage") if review else None
    if not isinstance(u, dict):
        return ""
    def num(k):
        v = u.get(k)
        return v if isinstance(v, (int, float)) and not isinstance(v, bool) and v >= 0 else 0
    tin = num("input_tokens") + num("cache_creation_input_tokens") + num("cache_read_input_tokens")
    tout = num("output_tokens")
    if not tin and not tout:
        return ""
    line = f"AI usage: {int(tin):,} input / {int(tout):,} output tokens"
    if num("cache_read_input_tokens"):
        line += f" ({int(num('cache_read_input_tokens')):,} from cache)"
    if num("cost"):
        line += f", about ${num('cost'):.2f}"
    return line + "."
...
def build_comment(ctx: Ctx, manifest: dict, review, *, artifact_url: str | None = None,
    ...
    if artifact_url:
        links.append(f"[⬇️ download the review bundle]({artifact_url})")
    if run_url:
    ...
    model = md_inline(review.get("model"), 60) if review and isinstance(review.get("model"), str) else ""
    foot = [""]
    usage = usage_line(review)
    if usage:
        foot.append(f"<sub>{usage}</sub><br>")
    foot.append("<sub>Generated by the component-review workflow"
                + (f" ({model})" if model else "")
                + ". AI findings can be wrong: check against the datasheet. The viewer may take a minute to "
                  "update after each push. This comment is updated in place.</sub>")
...
def main(argv=None) -> int:
    ...
    sys.stdout.write(build_comment(ctx, manifest, review, artifact_url=a.artifact_url, run_url=a.run_url, note=a.note))
    return 0
