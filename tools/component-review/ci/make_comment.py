...
from common import (MARKER, check_repo, check_sha, findings_of, item_review, load_site,  # noqa: E402
...
def details_block(ctx: Ctx, item: dict, review, inlined: set[str]) -> str:
    ...
    for f in fs[:25]:
        ...
        tag = " 💬" if finding_key(f) in inlined else ""
        cat = md_inline(f.get("category"), 30)
        msg = md_block(f.get("message"), 600).replace("\n", " ")
        lines.append(f"- {SEVERITY_ICON[sev]} **{sev}** · {cat}{where}{tag} — {msg}")
        if f.get("suggestion"):
            lines.append(f"  - _Suggestion:_ {md_block(f.get('suggestion'), 400).replace(chr(10), ' ')}")
    if len(fs) > 25:
...
def build_comment(ctx: Ctx, manifest: dict, review, *, artifact_url: str | None = None,
    ...
    foot = ["", "<sub>Generated by the component-review workflow"
            + (f" using {md_inline(review.get('model'), 60)}" if review and review.get("model") else "")
