...
"""Headless smoke test + screenshots of the viewer (dev tool, needs `playwright`).
    ...
    python3 screenshot.py --site <OUT with viewer copied in> --shots <dir> [--dark]

Serves --site on a local port, opens every view mode, fails (exit 1) on any page error,
console error or failed request, and writes PNG screenshots to --shots.
"""
import argparse
import functools
import http.server
...
def main():
    ...
    with sync_playwright() as p:
        ...
        for item in manifest["items"]:
            ...
            if page.locator(".tab[data-tab='3d']").count():
